...
#print(f'Código do pagamento : {dinheiro}')

mes = pd.date_range('20230605', periods=4, freq= 'M')
dia = pd.date_range('20231024', periods=4, freq= 'D')
'''
relatorio = pd.DataFrame({" CODIGO":range(4),
                    #"DATA":pd.Timestamp('20231024'),
                    "DT COMPRA":mes,
                    "PARCELAS":pd.Series(random.randint(1, 4) , index=list(range(4)), dtype='float32'),
                    "QNT":np.array([3]*4, dtype='int32'),
                    "PAGAMENTO":pd.Categorical([dinheiro, cartaocredito, pix, cartaodebito]),
                    "LOJA":'LOJA SGA',
                    "DT VENDA":dia,
                    "TOTAL" : pd.Series(random.randint(199,999) * np.array([3]*4)),
                    })
'''
#print(relatorio)
